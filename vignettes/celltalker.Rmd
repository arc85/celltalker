...
- Create circos plots of unique ligands/receptor interactions within each group
...
p3 <- DimPlot(ser.obj,reduction="umap",group.by="RNA_snn_res.0.5",label=T)
...
consistent.lig.recs
pull(consistent.lig.recs[1,2])[[1]]
pull(unnest(consistent.lig.recs[1,2])[1,2])[[1]]
```
...
# Identifying and visualizing ligand/receptor pairs in a group
...
unique.tonsil <- unique_interactions(put.int,group1="tonsil",group2="pbmc",interact.for)

for.circos <- put.int.sub[pull(unique.tonsil[1,2])[[1]]]

circos_plot(interactions=for.circos,clusters=defined.clusters)

put.int.sub <- pull(put.int[1,2])[[1]]
for.circos2 <- put.int.sub[pull(unique.tonsil[2,2])[[1]]]

circos_plot(interactions=for.circos2,clusters=defined.clusters)
