...
test_that("structure of result", {
  expect_type(pbmc_joint_scramble,"list")
})

test_that("dimensions", {
  expect_equal(dim(pbmc_joint_scramble),c(6100,4))
})

test_that("expected value in row 1 column 1", {
  expect_match(pbmc_joint_scramble[1,1] %>% pull(),"CD8 T cells_CD8 T cells")
})

test_that("value of row 6100 column 1", {
  expect_match(pbmc_joint_scramble[6100,1] %>% pull(),"RBCs_RBCs")
})
