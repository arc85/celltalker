## Test formatting of ligand and receptor means
# Create dataset
...
test_that("structure of result", {
  expect_type(pbmc_joint_means,"list")
})

test_that("dimensions", {
  expect_equal(dim(pbmc_joint_means),c(100,62))
})

test_that("expected value in row 1 column 1", {
  expect_match(pbmc_joint_means[1,1] %>% pull(),"CD8 T cells_CD8 T cells")
})

test_that("value of row 100 column 1", {
  expect_match(pbmc_joint_means[100,1] %>% pull(),"RBCs_RBCs")
})
